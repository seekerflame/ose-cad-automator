...
def generate_sheet_goods_section(parts):
    ...
    if plywood_count > 0:
        ...
        lines.append("| Part | Qty | Size | Notes |")
        lines.append("|------|-----|------|-------|")
        lines.append(f"| PT Plywood | {plywood_count} | 4' × 8' | Bottom layer - moisture barrier |")
        lines.append("")
    ...
    if osb_count > 0:
        ...
        lines.append("| Part | Qty | Size | Notes |")
        lines.append("|------|-----|------|-------|")
        lines.append(f"| OSB | {osb_count} | 4' × 8' | Top subfloor surface |")
        lines.append("")
...
def generate_insulation_section(parts):
    ...
    if insulation_count > 0:
        ...
        lines.append("| Part | Qty | Size | Notes |")
        lines.append("|------|-----|------|-------|")
        lines.append(f"| R-30 Batt Insulation | {insulation_count} | 16\" × 93\" | Between joists |")
...
def generate_assembly_steps(parts, width_ft, length_ft):
    ...
    lines.append("4. Use framing square to ensure 90° at each connection")
    lines.append("")
    
    lines.append("**Step 3: Square the Frame**")
    lines.append("1. Measure diagonals (corner to corner)")
    lines.append("2. Adjust until diagonals are equal (±1/8\")")
    ...
    lines.append("> ✅ **QC Checkpoint**: Frame should be square, level, and all joists secured.")
    lines.append("")
    ...
    if plywood_count > 0:
        ...
        lines.append("1. Start at one end with full 4'×8' sheet")
        ...
        lines.append("> ✅ **QC Checkpoint**: All sheathing edges supported by joists. No gaps > 1/8\".")
        lines.append("")
    ...
    if insulation_count > 0:
        ...
        lines.append("")
        lines.append("> ⚠️ **Important**: Do not leave gaps. Insulation should fit snugly but not be compressed.")
        lines.append("")
    ...
    if osb_count > 0:
        ...
        lines.append("> ✅ **QC Checkpoint**: Walk entire surface. No squeaks or soft spots.")
        lines.append("")
...
def generate_diagrams(width_ft, length_ft, has_water_inlet=False):
    ...
    lines.append("---")
    lines.append("")
    lines.append("## 📐 Reference Diagrams")
    lines.append("")
    ...
    lines.append("```")
    lines.append("        ┌────────────────────────────────────┐")
    lines.append("        │      OSB (3/4\")                    │  ← Subfloor")
    lines.append("        ├────┬─────────────────┬─────────────┤")
    lines.append("        │2x8 │  INSULATION     │ 2x8        │  ← Joists + Batt")
    lines.append("        │    │  R-30 BATT      │            │")
    lines.append("        ├────┴─────────────────┴─────────────┤")
    lines.append("        │      PT PLYWOOD (3/4\")             │  ← Moisture Barrier")
    lines.append("        └────────────────────────────────────┘")
    lines.append("```")
    lines.append("")
...
def weave_platinum_instructions(json_path, output_path=None):
    ...
    print(f"✅ SUCCESS: Generated PLATINUM instructions at {output_path}")
